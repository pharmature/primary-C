...
// assert - 断言
// 指针使用之前可以用 assert(p) 检查指针的有效性


// const 修饰指针变量
// const 在 * 前 - const char* arr
// 表示 *arr 指向的内容不能通过操作指针改变，但是 arr 可以改变储存的地址

// const 在 * 后 - char* const arr
// 表示 arr 储存的地址不能改变，但是 *arr 指向的内容可以通过操作指针改变
